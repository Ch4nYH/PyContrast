...
class RandomCropSlices(object):
	...
	def __call__(self, sample):
		...
		for i in range(self.sample_num):
			...
			output_image[i] = image[i * w // 4:i * w // 4 + label.shape[0] // 4, h1:h1+self.output_size, d1:d1+self.output_size]
			output_label[i] = label[i * w // 4:i * w // 4 + label.shape[0] // 4, h1:h1+self.output_size, d1:d1+self.output_size]
